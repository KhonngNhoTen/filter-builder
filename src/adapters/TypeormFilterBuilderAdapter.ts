import { Brackets, DataSource, Repository, SelectQueryBuilder, WhereExpressionBuilder } from "typeorm";
...
export class TypeormFilterBuilderAdapter<T extends object> extends FilterBuilderAdapter<T> {
  ...
  constructor(entity: T, page: number, limit?: number, alias?: string, options?: TypeormFilterBuilderAdapterOptions) {
    if (!options?.dataSource) throw new Error("You must set dataSource when using TypeOrmAdapter");
    const repo = options.dataSource.getRepository(entity as object & { name: string; type: T });
  ...
  handleCondition(conditionData: ConditionData): void {
    this.setConditions(this.selectQueryBuilder, [conditionData], this.getWhereMethodName());
  }
  ...
  async handleRun() {
    ...
    const [total, items] = await Promise.all([this.selectQueryBuilder.getCount(), cloneSqlBuilder.getMany()]);
    return {
      total,
      items,
    };
  ...
  handleJoin(dataJoin: JoinData): void {
    const joinMethod = dataJoin.required ? "innerJoinAndSelect" : "leftJoinAndSelect";
    this.selectQueryBuilder[joinMethod](dataJoin.path, dataJoin.path);
    if (dataJoin.conditions) dataJoin.conditions.forEach((val) => this.handleCondition(val));

    // Handle select attribute
    if (dataJoin.attributes) this.handleSelect(dataJoin.attributes, dataJoin.path);
  ...
  getColumns(target?: any): Record<string, any> {
    const repository = target ? this.datasource.getRepository(target) : this.repository;
    return repository.metadata.columns.reduce((pre, val) => ({ ...pre, [val.propertyName]: val }), {});
  }
    ...
    this.selectQueryBuilder[whereMethod](
      new Brackets((qb) => {
        ...
        for (let i = 1; i < subFilters.length; i++)
          qb[logicalMethod](new Brackets((_qb) => this.setConditions(_qb, subFilters[i].conditionData.conditions)));
      }),
    );
  }
  ...
  protected setConditions(qb: WhereExpressionBuilder, data: ConditionData[], forceWhereMethod?: "where" | "andWhere") {
   ...
   * - Gen String-query
   */
  protected parseCondition2Query(conditionData: ConditionData): [string, object] {
    let stringQuery = "";
    ...
    else if (operator === "BETWEEN") stringQuery = `${columnName} BETWEEN :${nameParams[0]} AND :${nameParams[1]}`;
  ...
  protected formatParams(conditionData: ConditionData) {
    const col = this.aliasColumnName(this.uniqueName(conditionData.columnName), conditionData.path);
    if (conditionData.operator === "BETWEEN" || (conditionData.operator !== "IN" && Array.isArray(conditionData.params)))
      return {
        [`${col}_1`]: conditionData.params[0],
        [`${col}_2`]: conditionData.params[1],
      };
    if (conditionData.operator === "ILIKE" || conditionData.operator === "LIKE") return { [col]: `%${conditionData.params}%` };

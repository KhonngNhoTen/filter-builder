...
import { ConditionData, JoinData, LogicalOperator, OperatorEnum, QueryData } from "./type";
...
export class SubFilter<T> extends BaseFilter {
  ...
  constructor(queryData: QueryData, adapter: FilterBuilderAdapter<T>, path?: string, target?: any, config?: FilterBuilderConfig) {
    ...
    this.conditionData.target = target ? target : this.adapter.getTargetByPath(this.conditionData.path);
  }
  ...
  protected select(attributes?: string[], skips?: string[] | "*"): void {
    ...
    if (attributes) select = attributes;
      ...
      const columns = Object.keys(this.adapter.getColumns(this.conditionData.target));
